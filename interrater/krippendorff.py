"""
SOURCE: https://github.com/pln-fing-udelar/fast-krippendorff/blob/master/krippendorff/krippendorff.py  # noqa
...
This module provides a function to compute the Krippendorff's alpha statistical
measure of the agreement achieved
...
def _coincidences(value_counts, value_domain, dtype=np.float64):
    ...
    diagonals = np.tile(np.eye(len(value_domain)), (len(value_counts), 1, 1)) \
        * value_counts.reshape(
        (value_counts.shape[0], 1, value_counts.shape[1]))
    unnormalized_coincidences = value_counts_matrices * \
        value_counts_matrices.transpose((0, 2, 1)) - diagonals
    return np.sum(np.divide(unnormalized_coincidences, (
            pairable - 1).reshape((-1, 1, 1)), dtype=dtype), axis=0)
...
def _random_coincidences(value_domain, n, n_v):
    ...
    return (n_v_column.dot(n_v_column.T) - np.eye(
        len(value_domain)) * n_v_column) / (n - 1)
...
def _distance_metric(level_of_measurement):
    ...
    level_of_measurement : string or callable
        ...
        It must be one of 'nominal', 'ordinal', 'interval', 'ratio'
        or a callable.
    Returns
    -------
    metric : callable
...
def _reliability_data_to_value_counts(reliability_data, value_domain):
    ...
    reliability_data : ndarray, with shape (M, N)
        Reliability data matrix which has the rate the i coder gave to the
        j unit, where M is the number of raters
    ...
    """
    return np.array([
        [sum(1 for rate in unit if rate == v) for v in value_domain]
        for unit in reliability_data.T])


def alpha(
        reliability_data=None, value_counts=None, value_domain=None,
        level_of_measurement='interval', dtype=np.float64):
    """Compute Krippendorff's alpha.
    See https://en.wikipedia.org/wiki/Krippendorff%27s_alpha for
    more information.
    Parameters
    ----------
    reliability_data : array_like, with shape (M, N)
        Reliability data matrix which has the rate the i coder gave to the j
        unit, where M is the number of raters
        and N is the unit count.
        Missing rates are represented with `np.nan`.
    ...
    value_domain : array_like, with shape (V,)
        Possible values the units can take.
        If the level of measurement is not nominal, it must be ordered.
        If `reliability_data` is provided, then the default value is the
        ordered list of unique rates that appear.
        Else, the default value is `list(range(V))`.
    level_of_measurement : string or callable
        Steven's level of measurement of the variable.
        It must be one of 'nominal', 'ordinal', 'interval', 'ratio' or
        a callable.
    dtype : data-type
        Result and computation data-type.
    Returns
    ...
    ...                     [np.nan, np.nan, 2, 1, 3, 4, 4, np.nan, 2, 1, 1, 3, 3, np.nan, 4]]  # noqa
    ...
    if (reliability_data is None) == (value_counts is None):
        raise ValueError("Either reliability_data or value_counts must be provided, but not both.")  # noqa

    # Don't know if it's a list or numpy array. If it's the latter, the
    # truth value is ambiguous. So, ask for None.
    if value_counts is None:
        if type(reliability_data) is not np.ndarray:
            reliability_data = np.array(reliability_data)

        value_domain = value_domain or np.unique(reliability_data[~np.isnan(
            reliability_data)])

        value_counts = _reliability_data_to_value_counts(
            reliability_data, value_domain)
    else:  # elif reliability_data is None
        if value_domain:
            assert value_counts.shape[1] == len(value_domain), \
                "The value domain should be equal to the number of columns of value_counts."  # noqa
        else:
...
def main():
    ...
    reliability_data_str = (
        "*    *    *    *    *    3    4    1    2    1    1    3    3    *    3",  # coder A  # noqa
        "1    *    2    1    3    3    4    3    *    *    *    *    *    *    *",  # coder B  # noqa
        "*    *    2    1    3    4    4    *    2    1    1    3    3    *    4",  # coder C  # noqa
    )
    ...
    print("Krippendorff's alpha for interval metric: ", alpha(
